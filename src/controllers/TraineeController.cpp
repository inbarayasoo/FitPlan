...
void register_trainee_routes(app::FitPlanApp& app,
    ...
    // GET /api/trainees ------------------------------------------------------
    CROW_ROUTE(app, "/api/trainees")([&app, &roster](const crow::request& req) {
        try {
            const auto& ctx =
                app.template get_context<middleware::JwtAuthMiddleware>(req);
            const util::TokenClaims claims = http::require_role(ctx, "coach");
            return dto::trainee_list_response(
                roster.list_trainees(claims.user_id));
    ...
    CROW_ROUTE(app, "/api/trainees")
        .methods(crow::HTTPMethod::Post)(
            [&app, &users, &roster](const crow::request& req) {
                try {
                    const auto& ctx =
                    ...
                    const dto::AttachTraineeRequest body =
                        dto::parse_attach_trainee_request(req.body);

                    const std::optional<models::User> user =
                        users.find_by_email(body.email);
                    if (!user) {
