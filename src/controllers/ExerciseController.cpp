...
models::Exercise owned_or_404(repositories::ExerciseRepository& repo, std::int64_t id,
                              std::int64_t coach_id) {
    const std::optional<models::Exercise> found = repo.find_by_id(id);
    if (!found || found->coach_id != coach_id) {
        throw http::ApiError(http::ApiErrorKind::kNotFound, "no exercise with that id");
    }
    return *found;
}
...
void register_exercise_routes(app::FitPlanApp& app, repositories::ExerciseRepository& exercises) {
    ...
    // GET /api/exercises ------------------------------------------------------
    CROW_ROUTE(app, "/api/exercises")
    ([&app, &exercises](const crow::request& req) {
        try {
            const auto& ctx = app.template get_context<middleware::JwtAuthMiddleware>(req);
            const util::TokenClaims claims = http::require_role(ctx, "coach");

            const std::vector<models::Exercise> rows = exercises.list_by_coach(claims.user_id);
    ...
    CROW_ROUTE(app, "/api/exercises/<int>")
        .methods(crow::HTTPMethod::Put)([&app, &exercises](const crow::request& req, int id) {
            try {
                const auto& ctx = app.template get_context<middleware::JwtAuthMiddleware>(req);
                const util::TokenClaims claims = http::require_role(ctx, "coach");
    ...
    CROW_ROUTE(app, "/api/exercises/<int>")
        .methods(crow::HTTPMethod::Delete)([&app, &exercises](const crow::request& req, int id) {
            try {
                const auto& ctx = app.template get_context<middleware::JwtAuthMiddleware>(req);
                const util::TokenClaims claims = http::require_role(ctx, "coach");
