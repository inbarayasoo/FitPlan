...
void require_exercise_on_active_plan(repositories::PlanRepository& plans,
    ...
    const std::optional<models::WorkoutPlan> plan =
        plans.find_active_for_trainee(trainee_id);
    if (!plan) {
    ...
    throw http::ApiError(http::ApiErrorKind::kNotFound,
                         "that exercise is not on your active plan");
}

}  // namespace

void register_exercise_note_routes(
    app::FitPlanApp& app, repositories::ExerciseNoteRepository& notes,
    repositories::PlanRepository& plans,
    repositories::PlanItemRepository& plan_items) {
    // GET /api/my/notes -------------------------------------------------------
    CROW_ROUTE(app, "/api/my/notes")([&app, &notes](const crow::request& req) {
        try {
            const auto& ctx =
                app.template get_context<middleware::JwtAuthMiddleware>(req);
            const util::TokenClaims claims = http::require_role(ctx, "trainee");
            return dto::note_list_response(
                notes.list_for_trainee(claims.user_id));
    ...
    CROW_ROUTE(app, "/api/my/notes/<int>")
        .methods(crow::HTTPMethod::Put)(
            [&app, &notes, &plans, &plan_items](const crow::request& req,
                                                int exercise_id) {
                try {
                    const auto& ctx =
                    ...
                    require_exercise_on_active_plan(plans, plan_items,
                                                    claims.user_id, exercise_id);
                    return dto::note_response(
                        200,
                        notes.upsert(claims.user_id, exercise_id, body));
                } catch (const std::exception& ex) {
                    return http::problem_response_for(ex);
                }
            });
