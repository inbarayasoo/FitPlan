...
models::WorkoutSession SessionService::owned_session_or_throw(std::int64_t trainee_id,
                                                              std::int64_t session_id) {
    const std::optional<models::WorkoutSession> found = sessions_.find_by_id(session_id);
    if (!found || found->trainee_id != trainee_id) {
        throw SessionError(SessionErrorKind::kNotFound, "no session with that id");
    }
    return *found;
}

PlanWithItems SessionService::active_plan_for(std::int64_t trainee_id) {
    const std::optional<models::WorkoutPlan> plan = plans_.find_active_for_trainee(trainee_id);
    if (!plan) {
        throw SessionError(SessionErrorKind::kNotFound, "you have no active plan");
    }
...
void SessionService::validate_sets(std::int64_t trainee_id,
    ...
    for (const SessionSetInput& s : sets) {
        ...
        if (s.reps && *s.reps < 0) {
            throw SessionError(SessionErrorKind::kInvalidInput, "reps must not be negative");
        }
        if (s.weight && *s.weight < 0.0) {
            throw SessionError(SessionErrorKind::kInvalidInput, "weight must not be negative");
        }
        if (s.rpe && (*s.rpe < 0.0 || *s.rpe > 10.0)) {
            throw SessionError(SessionErrorKind::kInvalidInput, "rpe must be between 0 and 10");
        }
    }
}

void SessionService::write_sets(std::int64_t session_id, const std::vector<SessionSetInput>& sets) {
...
SessionWithSets SessionService::log_session(std::int64_t trainee_id, const SessionInput& in) {
...
std::vector<SessionWithSets> SessionService::list_sessions(std::int64_t trainee_id) {
...
SessionWithSets SessionService::get_session(std::int64_t trainee_id, std::int64_t session_id) {
    const models::WorkoutSession s = owned_session_or_throw(trainee_id, session_id);
    return SessionWithSets{s, session_sets_.list_by_session(session_id)};
}

SessionWithSets SessionService::update_session(std::int64_t trainee_id, std::int64_t session_id,
                                               const SessionPatch& patch) {
    models::WorkoutSession session = owned_session_or_throw(trainee_id, session_id);
...
void SessionService::delete_session(std::int64_t trainee_id, std::int64_t session_id) {
...
std::vector<PrescribedItem> SessionService::prescribed_for(std::int64_t trainee_id) {
