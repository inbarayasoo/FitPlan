...
protected:
    void SetUp() override {
        ...
        fitplan::controllers::register_exercise_note_routes(app_, notes_,
                                                            plan_repo_,
                                                            plan_items_);
        ...
        trainee_id_ = json_number(req("GET", "/api/auth/me", "", trainee_).body,
                                  "id");
        ...
        ASSERT_EQ(req("POST", "/api/plans/" + std::to_string(plan_id) + "/assign",
                      "", coach_)
                      .status,
                  200);
    }
    ...
    long long make_exercise(const std::string& name) {
        auto r = req("POST", "/api/exercises",
                     R"({"name":")" + name + R"("})", coach_);
        EXPECT_EQ(r.status, 201) << r.body;
...
TEST_F(ExerciseNoteFlowTest, PutListGetAndDelete) {
    ...
    auto created = req("PUT", path, R"({"body":"brace before the pull"})",
                       trainee_);
    ...
    auto updated = req("PUT", path, R"({"body":"brace, then sit back"})",
                       trainee_);
    ...
    EXPECT_EQ(req("GET", "/api/my/notes", "", other_).body.find("brace"),
              std::string::npos);
...
TEST_F(ExerciseNoteFlowTest, RejectsBlankBodyOffPlanAndWrongRole) {
    EXPECT_EQ(req("PUT", "/api/my/notes/" + std::to_string(ex_on_),
                  R"({"body":"   "})", trainee_)
                  .status,
              400);

    // ex_off_ exists but is not on the active plan
    EXPECT_EQ(req("PUT", "/api/my/notes/" + std::to_string(ex_off_),
                  R"({"body":"x"})", trainee_)
                  .status,
              404);
